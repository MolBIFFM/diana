...
def main():
    ...
    parser.add_argument(
        "-c", "--configuration", help="YAML configuration file", required=True
    )
    parser.add_argument("-n", "--network", help="protein-protein interaction network")
    ...
    for entry in configuration.get("PTM", {}):
        for protein, sites in ppi_network.add_proteins_from_excel(
            ...
            protein_id_format=modify.MODIFY.get(entry["protein format"], lambda x: x),
            position_format=modify.MODIFY.get(entry["position format"], lambda x: x),
    ...
    if configuration.get("PPI"):
        ...
        if configuration["PPI"].get("IntAct"):
            for (
                ...
                interaction_detection_methods=configuration["PPI"]["IntAct"].get(
        ...
        if configuration["PPI"].get("STRING"):
            for (
                ...
                neighborhood=configuration["PPI"]["STRING"].get("neighborhood", 0.0),
                ...
                cooccurence=configuration["PPI"]["STRING"].get("cooccurence", 0.0),
                homology=configuration["PPI"]["STRING"].get("homology", 0.0),
                coexpression=configuration["PPI"]["STRING"].get("coexpression", 0.0),
                coexpression_transferred=configuration["PPI"]["STRING"].get(
                    "coexpression transferred", 0.0
                ),
                experiments=configuration["PPI"]["STRING"].get("experiments", 0.7),
