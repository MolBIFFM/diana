...
for entry in configuration.get("PTM", {}):
    ppi_network.add_proteins_from_excel(entry["file"],
                                        entry["label"],
                                        entry["time"],
                                        num_replicates=entry.get(
                                            "replicates", 2),
                                        num_sites=entry.get("sites", 5))

if configuration.get("PPI"):
    if configuration["PPI"].get("BioGRID"):
        ppi_network.add_interactions_from_BioGRID(
    ...
    if configuration["PPI"].get("STRING"):
        ppi_network.add_interactions_from_STRING(
